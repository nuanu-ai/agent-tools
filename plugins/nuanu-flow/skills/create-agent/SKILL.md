...
## 6. Launch in the current Codex session

Use the bundled scripts without restarting Codex:

1. Resolve the active plugin root from the loaded skill path (`../..`). If the
   skill is not filesystem-backed, run `codex plugin list --json`, select the
   enabled Nuanu Flow plugin for the active environment, and use its exact
   `source.path`. Never guess a cache or build path.
...
5. Start `scripts/worker/worker.mjs` in the background with
   `NUANU_ADAPTER=codex-app-server`. The worker reads the protected credential
   written by enrollment.
